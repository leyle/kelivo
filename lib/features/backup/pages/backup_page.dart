...
void _sortBackupItems(List<BackupFileItem> items) {
  items.sort((a, b) {
...
class _BackupPageState extends State<BackupPage> {
  ...
  Future<List<BackupFileItem>> _loadRemoteBackups(BackupProvider vm) async {
    final list = await vm.listRemote();
    _sortBackupItems(list);
    return list;
  }
                  ...
                  children: [
                    ...
                    _iosNavRow(
                      ...
                      onTap: vm.busy
                          ? null
                          : () async {
                              final remoteItems = await _loadRemoteBackups(vm);
                              if (!mounted) return;
...
class _RemoteListSheet extends StatefulWidget {
  ...
  final List<BackupFileItem> initialItems;
  final bool initialLoading;
  final Future<List<BackupFileItem>?> Function(BackupFileItem) onDelete;
  final Future<void> Function(BackupFileItem) onRestore;

  @override
  State<_RemoteListSheet> createState() => _RemoteListSheetState();
}

class _RemoteListSheetState extends State<_RemoteListSheet> {
  late List<BackupFileItem> _items;
  late bool _loading;

  @override
  void initState() {
    super.initState();
    _items = List<BackupFileItem>.from(widget.initialItems);
    _loading = widget.initialLoading;
  }

  Future<void> _deleteItem(BackupFileItem item) async {
    setState(() => _loading = true);
    try {
      final updatedItems = await widget.onDelete(item);
      if (!mounted || updatedItems == null) return;
      setState(() => _items = List<BackupFileItem>.from(updatedItems));
    } finally {
                            ...
                            child: Container(
                              ...
                              child: Row(
                                children: [
                                  ...
                                  _SmallTactileIcon(
                                    icon: Lucide.Import,
                                    onTap: () => widget.onRestore(it),
                                  ),
                                  const SizedBox(width: 6),
                                  _SmallTactileIcon(
                                    icon: Lucide.Trash2,
                                    onTap: () => _deleteItem(it),
                                    baseColor: cs.error,
                                  ),
                                ],
                              ),
